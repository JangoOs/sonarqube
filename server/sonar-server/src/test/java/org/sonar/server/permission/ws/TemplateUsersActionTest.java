...
public class TemplateUsersActionTest {
  ...
  public void search_for_users_by_template_name() throws IOException {
    ...
    PermissionTemplateDto template1 = dbClient.permissionTemplateDao().insert(dbSession, newPermissionTemplateDto().setUuid("template-uuid-1"));
    PermissionTemplateDto template2 = dbClient.permissionTemplateDao().insert(dbSession, newPermissionTemplateDto().setUuid("template-uuid-2"));
    ...
    addUserToTemplate(newPermissionTemplateUser(USER, template1.getId(), user1.getId()));
    addUserToTemplate(newPermissionTemplateUser(USER, template1.getId(), user2.getId()));
    addUserToTemplate(newPermissionTemplateUser(ISSUE_ADMIN, template1.getId(), user1.getId()));
    addUserToTemplate(newPermissionTemplateUser(ISSUE_ADMIN, template1.getId(), user3.getId()));
    addUserToTemplate(newPermissionTemplateUser(USER, template2.getId(), user1.getId()));
    addUserToTemplate(newPermissionTemplateUser(USER, template2.getId(), user2.getId()));
    addUserToTemplate(newPermissionTemplateUser(USER, template2.getId(), user3.getId()));
    addUserToTemplate(newPermissionTemplateUser(ISSUE_ADMIN, template2.getId(), user1.getId()));
    commit();
  ...
  public void fail_if_template_does_not_exist() {
    ...
    newRequest(USER, "unknown-template-uuid")
      .execute();
  }
  ...
  public void fail_if_template_uuid_and_name_provided() {
    ...
    expectedException.expect(BadRequestException.class);
    newRequest(USER, template.getUuid())
  ...
  public void fail_if_not_logged_in() {
    ...
    newRequest(USER, template.getUuid()).execute();
  }
